...
export default function Customers() {
  ...
  const loadStatement = async (customer, from, to) => {
    const params = new URLSearchParams();
    if (from) params.append('from', from);
    if (to) params.append('to', to);

    const { data } = await api.get(
      `/customers/${customer._id}/statement${params.toString() ? `?${params.toString()}` : ''}`
    );
    setStatement(data);
  };
  ...
  const removeCustomer = async (customer) => {
    setConfirmMessage(`Delete customer ${customer.name}? This cannot be undone.`);
    setConfirmAction(() => () => performRemoveCustomer(customer._id));
    setConfirmOpen(true);
  };

  const performRemoveCustomer = async (id) => {
    try {
      await api.delete(`/customers/${id}`);
      setConfirmOpen(false);
      load();
    } catch (err) {
      setLoadError(err.response?.data?.message || 'Could not delete customer.');
      setConfirmOpen(false);
    }
  };
    ...
    { key: 'actions', label: '', align: 'right', render: (r) => (
        <div className="flex items-center justify-end gap-2">
          ...
          {canManage && (
            <button
              type="button"
              onClick={(e) => { e.stopPropagation(); removeCustomer(r); }}
  ...
  return (
    <PageLayout
      title="Customers"
      actions={canManage && (
        <button onClick={openCreate} className="flex items-center gap-1.5 bg-ink-900 hover:bg-ink-800 text-white text-sm px-3.5 py-2 rounded-lg">
